<template>
  <Card>
    <template #title>
      <img src="@/assets/screen/djmxhyb.png" class="w-[116px]" alt="" />
    </template>
    <template #content>
      <div ref="chart" class="w-full h-full"></div>
    </template>
  </Card>
</template>
<script setup>
import { onBeforeUnmount, onMounted, ref } from "vue";
import Card from "./Card.vue";
import * as echarts from "echarts";
import { getSchoolRanking } from "@/api/university-dataOverview";

const chart = ref(null);
let myChart;
const init = () => {
  myChart = echarts.init(chart.value);
  const option = {
    grid: {
      left: "8px",
      right: "-30px",
      bottom: "-20px",
      top: "24px",
      containLabel: true,
    },
    xAxis: {
      type: "value",
      show: false,
      axisLine: {
        show: false,
      },
      splitLine: {
        show: false,
      },
    },
    yAxis: [
      {
        type: "category",
        axisLine: {
          show: false,
        },
        axisTick: {
          show: false,
        },
        axisLabel: {
          show: false,
        },
      },
      {
        triggerEvent: true,
        show: true,
        // inverse: true,
        data: data.value?.map((item) => item?.activity || 0),
        axisLine: {
          show: false,
        },
        splitLine: {
          show: false,
        },
        axisTick: {
          show: false,
        },
        axisLabel: {
          interval: 0,
          color: "rgba(255,255,255,0.8)",
          align: "right",
          verticalAlign: "bottom",
          lineHeight: 30,
          fontSize: 13,
          padding: [0, 10, 0, 0],
          fontFamily: "BarlowCondensed-SemiBold",
          formatter: function (value, index) {
            return value;
          },
        },
      },
    ],
    series: [
      {
        type: "pictorialBar",
        symbol:
          "image://data:image/png;base64,",
        symbolSize: [16, 16],
        symbolOffset: [6, 0],
        z: 12,
        data: data.value?.map((item) => {
          return {
            value: item?.activity || 0,
            symbolPosition: "end",
          };
        }),
      },
      {
        type: "bar",
        showBackground: true,
        backgroundStyle: {
          color: "#193A96",
        },
        data: data.value?.map((item, index) => {
          return {
            value: item?.activity || 0,
            name: item?.schoolName,
            rank: item?.rank,
          };
        }),
        barWidth: "2px",
        itemStyle: {
          color: new echarts.graphic.LinearGradient(0, 0, 1, 0, [
            { offset: 0, color: "#537CE5" },
            { offset: 1, color: "#C8D7FF" },
          ]),
          borderRadius: [0, 0, 0, 0],
        },
        label: {
          show: true,
          formatter: (parmas) => {
            return parmas.name;
          },
          color: "rgba(255,255,255,0.8)",
          fontSize: 14,
          fontWeight: 400,
          position: [0, -20],
        },
      },
    ],
  };

  option && myChart.setOption(option);
};

const handleResize = () => {
  if (myChart) {
    myChart.resize();
  }
};
const data = ref([]);

onMounted(() => {
  getSchoolRanking().then((res) => {
    data.value = res.data || [];
    init();
  });
  window.addEventListener("resize", handleResize);
});

onBeforeUnmount(() => {
  window.removeEventListener("resize", handleResize);
  if (myChart) {
    myChart.dispose();
    myChart = null;
  }
});
</script>
